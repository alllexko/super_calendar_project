...
class SuperCalendarConfigurator(models.Model):
    ...
    def _clear_super_calendar_records(self):
        _logger.info('---------------_clear_super_calendar_records--------------- %s', self)
        """
    ...
    def generate_calendar_records(self):
        _logger.info('---------------generate_calendar_records--------------- %s', self)
        """
        ...
        for configurator in configurator_list:
            # _logger.info('---------------configurator--------------- %s', configurator)
            for line in configurator.line_ids:
                # _logger.info('---------------configurator_line--------------- %s', line)
    ...
    def _generate_record_from_line(self, line):
        # _logger.info('---------------_generate_record_from_line_self--------------- %s', self)
        # _logger.info('---------------_generate_record_from_line_line--------------- %s', line)
        """
        ...
        for record in values:
            # _logger.info('---------------record-11111111111-------------- %s', record)
            super_calendar_pool.create(values[record])

    @api.multi
    def _get_record_values_from_line(self, line):
        # _logger.info('---------------_get_record_values_from_line_self--------------- %s', self)
        # _logger.info('---------------_get_record_values_from_line_line--------------- %s', line)
        """
        ...
        for cur_rec in current_record_list:
            ...
            # if ((cur_rec[f_descr] or line.description_code) and
            #         cur_rec[f_date_stop], [f_date_stop]):
            #     duration = False    
            ...
            #     )
            #     _logger.info('---------------date_stop--------------- %s', date_stop)
